  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">求职身份</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">目标行业</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">简历编写 STAR 法则与量化成果指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 求职简历诊断与精修专家');
...
const metricsList = [
  {
    "key": "starMethodCompleteness",
    "label": "STAR法则应用度"
  },
  {
    "key": "quantifiedResultHighlight",
    "label": "成果数据量化度"
  },
  {
    "key": "hrKeywordsMatching",
    "label": "HR关键词匹配度"
  },
  {
    "key": "layoutProfessionalism",
    "label": "排版格式专业度"
  },
  {
    "key": "careerGrowthPotential",
    "label": "职业发展潜力值"
  }
];
const guideRules = [
  {
    "title": "STAR法则公式",
    "advice": "背景(Situation)+任务(Task)+行动(Action)+定量结果(Result)缺一不可",
    "avoid": "切忌只写负责过某项目而不写最终带来了什么成果"
  },
  {
    "title": "量化数据包装",
    "advice": "多用百分比、具体金额、完成天数、提升倍数等数字突出贡献",
    "avoid": "避免使用较多、良好、显著等模糊主观词汇"
  },
  {
    "title": "HR眼球3秒卡位",
    "advice": "将最核心的亮点与数据成果放在每段经历的前两行",
    "avoid": "切勿把不相关的实习经验放在简历最显著位置"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[JIANLI_SCORES\][\s\S]*?\[\/JIANLI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[JIANLI_SCORES\](.*?)\[\/JIANLI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('jianli_history_records');
...
const saveHistory = () => {
  localStorage.setItem('jianli_history_records', JSON.stringify(historyList.value));
};
